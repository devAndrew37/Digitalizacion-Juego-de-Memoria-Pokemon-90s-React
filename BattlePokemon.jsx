...
const Battle = ({ id1, id2, theme, showTransition, setShowTransition, onClose, setBattleMusic, setIsPlaying, setTurn, boardFlag, setBoardFlag, 
 ...
 const youWin = preloadedAudios["/assets/youwin.mp3"];
 const youLose = preloadedAudios["/assets/lose.mp3"];
 const select = preloadedAudios["/assets/select.mp3"];
 const superEffective = preloadedAudios["/assets/supereffective.mp3"];
 const runAway = preloadedAudios["/assets/runaway.mp3"];
...
const damageCalculation = async (move, player, enemy_move, enemy) => {
  ...
  const hitAudio = preloadedAudios["/assets/hit.mp3"];
  const audioAttack = preloadedAudios[`/assets/sfx_attacks/${move[0]}.mp3`];
  const atk1 = attack1;
  const def1 = defense1;
